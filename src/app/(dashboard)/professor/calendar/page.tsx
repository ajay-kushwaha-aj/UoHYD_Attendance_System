...
import { Clock, MapPin, Play, Calendar as CalendarIcon } from "lucide-react";
...
export default function ProfessorCalendarPage() {
  const { courses, currentProfessor } = useAttendance();
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-on-surface-variant mt-0.5">
            Department of Systems & Computational Biology • Weekly Lecture Plan
          </p>
        </div>
            ...
            <div className="space-y-2.5">
              {d.classes.length > 0 ? (
                d.classes.map((cls, idx) => (
                  <Card key={idx} className="p-3.5 space-y-2.5 hover:border-outline transition-colors text-xs border border-border">
                    ...
                    <div className="pt-2 border-t border-surface-container">
                      <Link href={`/professor/session/sess-today-01`}>
                        <Button variant="secondary" size="sm" className="w-full text-xs h-7 gap-1 font-semibold">
